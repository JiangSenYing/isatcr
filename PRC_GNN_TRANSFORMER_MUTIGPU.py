...
from common import build_save_training_data_name
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Device to use for the agent/env (cuda/cpu). Kept for backward compatibility.'
    )
    parser.add_argument(
        '--agent-device', type=str, default="cuda:0",
        help='Device to use for the GNN agent and environment, e.g. cuda:0.'
    )
    parser.add_argument(
        '--transformer-device', type=str, default="cuda:1",
        help='Device to use for the Transformer planner/trainer, e.g. cuda:1. Defaults to agent device.'
    )
    parser.add_argument(
...
def get_default_config() -> dict:
    ...
    return {
        ...
        'transformer': {
            ...
            'model_path': 'model_weights/transformer_forcase1.pth',
        },
        'environment': {
...
def main():
    ...
    config['environment']['save_training_data'] = build_save_training_data_name(config)
    print(f"save_training_data: {config['environment']['save_training_data']}")
    ...
    # 设置设备。环境会直接调用 q_net，因此环境和 agent 必须在同一设备上。
    agent_device_arg = args.agent_device or args.device
    if agent_device_arg:
        device = torch.device(agent_device_arg)
    else:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    transformer_device = torch.device(args.transformer_device) if args.transformer_device else device
    print(f"Using agent/env device: {device}")
    print(f"Using transformer device: {transformer_device}")
    ...
    for k in range(rounds):
        ...
        for t in range(n_steps):
            ...
            if phase == 'train':
                ...
                parts = None
                if transformer_trainer is not None:
                    for i in range(transformer_trainer.repeat):
                ...
                if (t + 1) % config['general']['print_cycle'] == 0:
                    ...
                    if parts is not None:
                        env.latest_transformer_losses = {
                            ...
                            'meo_exit_policy_loss': parts.get('meo_exit_policy_loss'),
